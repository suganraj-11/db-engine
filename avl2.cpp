...
int getMin(fstream &file, int offset) {
    Node node = readNode(file, offset);
    while (node.left_offset != -1) {
        node = readNode(file, node.left_offset);
    }
    return node.key;
}
...
int deleteNode(fstream &file, int offset, int key, int &node_count) {
    ...
    if (key < node.key)
        ...
        if (node.left_offset == -1 || node.right_offset == -1) {
            ...
            int min_key = getMin(file, node.right_offset);
            node.key = min_key;
            node.right_offset = deleteNode(file, node.right_offset, min_key, node_count);
        }
    }
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            cin >> key;
            root_offset = deleteNode(file, root_offset, key, node_count);
            writeRoot(file, root_offset);
