...
int findMin(fstream &file, int offset) {
    Node node = readNode(file, offset);
    while (node.left_offset != -1) {
        offset = node.left_offset;
        node = readNode(file, offset);
    }
    return offset;
}

int deleteNode(fstream &file, int offset, int key, int &node_count) {
    if (offset == -1) {
        return offset; // Node not found
    }

    Node node = readNode(file, offset);

    if (key < node.key) {
        node.left_offset = deleteNode(file, node.left_offset, key, node_count);
    }
    else if (key > node.key) {
        node.right_offset = deleteNode(file, node.right_offset, key, node_count);
    }
    else {
        // Node to be deleted is found
        if (node.left_offset == -1 || node.right_offset == -1) {
            int temp_offset = (node.left_offset != -1) ? node.left_offset : node.right_offset;
            if (temp_offset == -1) {
                temp_offset = offset;
                offset = -1;
            } else {
                node = readNode(file, temp_offset);
            }
            writeNode(file, offset, node);
            return temp_offset;
        }
            ...
            int successor_offset = findMin(file, node.right_offset);
            Node successor = readNode(file, successor_offset);

            node.key = successor.key;
            node.value = successor.value;
            node.right_offset = deleteNode(file, node.right_offset, successor.key, node_count);
        }
    }

    if (offset == -1) return offset;  // Return if the node was deleted

    updateHeight(file, offset);

    int balance = getBalance(file, offset);

    if (balance > 1 && getBalance(file, node.left_offset) >= 0) {
        return rotateRight(file, offset);
    }
    if (balance < -1 && getBalance(file, node.right_offset) <= 0) {
        return rotateLeft(file, offset);
    }
    if (balance > 1 && getBalance(file, node.left_offset) < 0) {
        node.left_offset = rotateLeft(file, node.left_offset);
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            cin >> key;
            root_offset = deleteNode(file, root_offset, key, node_count); // update root after deletion
            writeRoot(file, root_offset); // write the new root to the file
